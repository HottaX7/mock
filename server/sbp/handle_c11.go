...
func (h *Handler) HandleC11(w http.ResponseWriter, r *http.Request) {
	const op = "sbp.HandleC11"

	metrics.AddRequest("C11")
	...
	c11 := &entity.C11{}
	if err := xml.Unmarshal(bodyBytes, c11); err != nil {
		log.Error().Err(err).Str("op", op).Send()
		w.WriteHeader(http.StatusInternalServerError)
		return
	}

	if err := h.SBPUsecase.C11(ctx, c11); err != nil {
