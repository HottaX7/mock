...
func (h *Handler) HandleC13(w http.ResponseWriter, r *http.Request) {
	const op = "sbp.HandleC13"

	metrics.AddRequest("C13")
	...
	c13 := &entity.C13{}
	if err := xml.Unmarshal(bodyBytes, c13); err != nil {
		log.Error().Err(err).Str("op", op).Send()
		w.WriteHeader(http.StatusInternalServerError)
		return
	}

	if err := h.SBPUsecase.C13(ctx, c13); err != nil {
